...
- ihor-sokoliuk/mcp-searxng (481+ stars): Dominant. Search + URL read + caching + pagination. Node.js. Actively maintained (pushed Feb 22, 2026)
- SecretiveShell/searxng-mcp-server: Simplest setup, good starting point
- **web-research-assistant (6 stars):** Broadest feature set (13 tools) but lowest adoption. Python. Last commit Jan 2, 2026
- crawl4ai-rag-mcp (ToKiDoO): Docker all-in-one with search + crawl + vector DB + RAG
- searxng-crawl4ai-mcp (luxiaolei): Claims 3x faster than native tools, 100% scraping reliability
- 73+ total SearXNG MCP repos on GitHub (space increasingly crowded)

**Tier 3 - Multi-engine aggregators (growing category):**
- mcp-omnisearch: Unified interface across Tavily, Brave, Kagi, Perplexity, Exa
- one-search-mcp (yokingma): Unified SearXNG + Tavily + DuckDuckGo + Bing + agent-browser
- kindly-web-search-mcp-server (Shelpuk-AI): Supports Serper, Tavily, and SearXNG. Jan 2026

web-research-assistant's unique positioning is as a "developer research toolkit" rather than a search-only server. The package registry, GitHub, error translation, API docs, tech comparison, and service status tools have no equivalent in other SearXNG MCP servers. However, this breadth hasn't translated to adoption, likely because: (a) the dominant ihor-sokoliuk version is "good enough" for most search needs, (b) Docker setup friction, (c) zero marketing/community engagement, and (d) the emerging multi-provider aggregator tier (one-search-mcp, kindly-web-search) offers broader engine coverage without SearXNG lock-in.

**Note (Feb 23 re-verification):** No changes since initial report. Zero new commits, zero new stars, zero community mentions. The SearXNG MCP space has grown more crowded (73+ repos), and multi-provider aggregators are emerging as a new competitive tier. XDA Developers featured SearXNG MCP but referenced ihor-sokoliuk exclusively. Project appears dormant.
...
### Re-verification (2026-02-23)
- https://www.xda-developers.com/favorite-mcp-server-use-local-llm/ (SearXNG MCP featured, references ihor-sokoliuk)
- https://www.reddit.com/r/mcp/comments/1m5gts9/best_web_search_mcp_server/
- https://lobehub.com/mcp/elad12390-web-research-assistant
- https://medium.com/@shouke.wei/understanding-searxng-mcp-privacy-first-search-for-ai-a85bbdb46b59

---
*Generated by Research Agent on 2026-02-23. Re-verified 2026-02-23: no material changes.*
